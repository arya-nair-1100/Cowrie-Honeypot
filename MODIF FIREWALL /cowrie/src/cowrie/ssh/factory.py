...
class HoneyTrackControlProtocol(protocol.Protocol):
    """
    Local control channel used by HoneyTrack to terminate
    an active Cowrie SSH session by session ID.
    """
    ...
    def dataReceived(self, data):
        ...
        session = shellTransport.ACTIVE_SESSIONS.get(session_id)
...
class HoneyTrackControlFactory(protocol.Factory):
    protocol = HoneyTrackControlProtocol
...
class CowrieSSHFactory(factory.SSHFactory):
    ...
    def __init__(self, backend, pool_handler):
        ...
        self.honeytrack_socket = "/tmp/honeytrack_cowrie.sock"
        self.honeytrack_listener = None
        self.publicKeys = {}
        self.services = {
            b"ssh-userauth": (
                ProxySSHAuthServer
    ...
    def startFactory(self) -> None:
        ...
        # Local-only control socket for HoneyTrack.
        if os.path.exists(self.honeytrack_socket):
            os.unlink(self.honeytrack_socket)

        self.honeytrack_listener = reactor.listenUNIX(
            self.honeytrack_socket,
            HoneyTrackControlFactory(),
            mode=0o600
        )

        self._log.info("HoneyTrack control socket ready")
        self._log.info("Ready to accept SSH connections")

    def stopFactory(self) -> None:

        if self.honeytrack_listener:
            self.honeytrack_listener.stopListening()
            self.honeytrack_listener = None
