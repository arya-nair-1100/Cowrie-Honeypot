...
def terminate_session(session_id):
    """
    Terminate the active Cowrie session using
    the local HoneyTrack control socket.
    """

    import socket

    socket_path = "/tmp/honeytrack_cowrie.sock"
    ...
    try:
        ...
        if response == "OK":
            print("Action: Cowrie session terminated successfully.")

        elif response == "NOT_FOUND":
            print("Action: Cowrie session was not found.")

        else:
            print(f"Action: Cowrie returned: {response}")

    except Exception as e:
        print(f"Action: Failed to terminate Cowrie session: {e}")
